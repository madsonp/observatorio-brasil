...
function severity(mag) {
  if (mag >= 6) return { key: 'critical', label: 'Crítica' };
  if (mag >= 4.5) return { key: 'high', label: 'Alta' };
  if (mag >= 2.5) return { key: 'moderate', label: 'Moderada' };
...
function isBrazilRegion(feature) {
  ...
  return lat >= -45 && lat <= 15 && lon >= -85 && lon <= -25;
}
...
function renderMap() {
  ...
  state.filtered = state.features.filter(feature => {
    ...
    return mag >= minMag && (!brazilOnly || isBrazilRegion(feature));
  });
  ...
  state.filtered.forEach(feature => {
    ...
    const marker = L.circleMarker([lat, lon], {
      ...
      <strong>${feature.properties.place || 'Local não informado'}</strong><br>
      Magnitude: ${mag.toFixed(1)}<br>
      Profundidade: ${Number(depth || 0).toFixed(1)} km<br>
      ${formatDate(feature.properties.time)}
    `);
    markerLayer.addLayer(marker);
  });

  const maxMag = state.filtered.length
    ? Math.max(...state.filtered.map(f => Number(f.properties.mag || 0))).toFixed(1)
    : '—';
  ...
  container.innerHTML = features.map(feature => {
    ...
    return `
      <article class="event-card">
        ...
        <div>
          <strong>${feature.properties.place || 'Local não informado'}</strong>
          <span>${sev.label} • ${depth.toFixed(1)} km de profundidade</span>
          <small>${formatDate(feature.properties.time)} • há ${relativeTime(feature.properties.time)}</small>
        </div>
      </article>`;
  }).join('');
}
...
function renderLocationDashboard(location) {
  ...
  const maxEvent = events.reduce((best, event) => Number(event.properties.mag || 0) > Number(best?.properties.mag || -1) ? event : best, null);
  const latest = events[0] || null;
  const averageDepth = events.length
    ? events.reduce((sum, event) => sum + Number(event.geometry.coordinates[2] || 0), 0) / events.length
    : 0;

  const counts = { low: 0, moderate: 0, high: 0, critical: 0 };
  events.forEach(event => counts[severity(Number(event.properties.mag || 0)).key]++);
  const mainKey = Object.entries(counts).sort((a, b) => b[1] - a[1])[0][0];
  const main = severity(mainKey === 'critical' ? 6 : mainKey === 'high' ? 4.5 : mainKey === 'moderate' ? 2.5 : 0);
  ...
  el('local-max-mag').textContent = maxEvent ? Number(maxEvent.properties.mag || 0).toFixed(1) : '—';
  el('local-max-place').textContent = maxEvent ? maxEvent.properties.place : 'Nenhum evento';
  el('local-main-severity').textContent = events.length ? main.label : '—';
  ...
  el('local-latest-place').textContent = latest ? latest.properties.place : 'Nenhum evento';
  ...
  const labels = { low: 'Baixa', moderate: 'Moderada', high: 'Alta', critical: 'Crítica' };
